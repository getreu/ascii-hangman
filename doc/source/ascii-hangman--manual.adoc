...
_ASCII-Hangman_ is a paper and pencil guessing game for two or more players.  One player
...
progressively disclosed. There are 30 built in ASCII-Art images from which one
is chosen randomly at the beginning of the game. Alternatively own ASCII-Art
...
The web-app version does not need to be installed. Just navigate to
https://blog.getreu.net/projects/ascii-hangman/web-app/[ASCII-Hangman Web-App].
...
_ASCII-Hangman_ starts, its will read the modified list of secrets.

The Web-App version of _ASCII-Hangman_ works similar. Here the programm starts
...
include::../../../src/main.rs[lines="66..107"]
...
it can be defined it the configuration file.
...
All versions of _ASCII-Hangman_ run locally on your computer and do not
collect or transfer any data. 

This applies in particular to the web application version of _ASCII-Hangman_:
due to its Web-Assembly-technology, the programm runs - once started - entirely
in your Web-Browser's sandbox without any networks connection.
